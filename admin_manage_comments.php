...
try {
    // 🔹 Base queries
    $query = "SELECT c.*, u.first_name, u.last_name, u.email, p.title AS post_title, c.is_flagged
              FROM comments c
    ...
    $search_flagged = isset($_GET['flagged']) ? $_GET['flagged'] : '';
    if ($search_flagged === '1' || $search_flagged === '0') {
        $query .= " AND c.is_flagged = :flagged";
        $count_query .= " AND c.is_flagged = :flagged";
        $params[':flagged'] = $search_flagged;
    }
                        ...
                        <form method="GET" class="row g-2 align-items-end">
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="flagged" class="form-select">
                                    ...
                                    <option value="1" <?= isset($_GET['flagged']) && $_GET['flagged'] == '1' ? 'selected' : ''; ?>>Flagged</option>
                                    <option value="0" <?= isset($_GET['flagged']) && $_GET['flagged'] == '0' ? 'selected' : ''; ?>>Not Flagged</option>
                                </select>
                            </div>
                            <div class="col-md-2 d-flex gap-2">
                                <button type="submit" class="btn btn-primary flex-grow-1"><i class="fas fa-filter me-1"></i> Filter</button>
                                <button type="button" id="clearFilter" class="btn btn-secondary flex-grow-1"><i class="fas fa-redo me-1"></i> Reset</button>
                            </div>
                        </form>
                    </div>
                </div>
                                ...
                                <?php if (!empty($comments)): ?>
                                    <?php foreach ($comments as $c): ?>
                                        <tr>
                                            ...
                                            <td><?= htmlspecialchars($c['post_title'] ?? 'Deleted Post'); ?>
                                                <?php if (!empty($c['is_flagged'])): ?>
